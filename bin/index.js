    ...
    try {
      ...
      if (answer) {
        ...
        if (cnt === 3) {
          log("\n");
          log(
            chalk.red.bold(`Incorrect answer again, lifeline exausted!`),
            `\n\nquiting now.`
          );
...
function printRelatedWord(
  ...
  relatedWords.length > 0 &&
    ...
    relatedWords[0].words.length > 0 &&
    log(
      relatedWords[0].words[
        getRandom(
          `${
            relatedWords[0].words.length > 1
              ? relatedWords[0].words.length - 1
              : 1
          }`
        )
  ...
  shouldExit &&
    relatedWords.length == 0 &&
    printRelatedWord(result, "synonym", !shouldExit, isFullRequired, true);
